...
export function shuffleGrammarExercises(
...
export function createStableGrammarOrder(
  exercises: GrammarExercise[],
  profileId: string,
  mode: GrammarPracticeMode,
  level: GrammarLevel,
): string[] {
  return shuffleGrammarExercises(exercises, `${profileId}:grammar:${mode}:${level}`).map(
